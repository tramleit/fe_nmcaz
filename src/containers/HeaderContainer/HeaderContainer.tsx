...
const HeaderContainer: FC<HeaderContainerProps> = ({ className = "" }) => {
  ...
  }, [location, store.getState().app.isAuthenticated]);
          ...
          {homePages.map((home) => {
            return (
              <Link
                key={home.slug}
                to={home.slug}
                className={`mr-2 my-1 py-1.5 px-3.5 flex items-center  rounded-full font-medium text-xs cursor-pointer select-none ${location.pathname === home.slug
                  ? "bg-black text-white shadow-black/10 shadow-lg"
                  : "border border-neutral-300 dark:border-neutral-700 hover:border-neutral-400 dark:hover:border-neutral-500"
                  }`}
              >
        ...
        <div className="fixed right-3 top-1/4 z-40 flex items-center">
          <Popover className="relative">
            {({ open }) => (
              <>
                <Popover.Button
                  className={`p-3 bg-white hover:bg-neutral-100 dark:bg-primary-6000 dark:hover:bg-primary-700 rounded-xl [ nc-custom-shadow-1 ] border border-neutral-200 dark:border-primary-6000 z-10 focus:outline-none ${open ? " focus:ring-2 ring-primary-500" : ""
                    }`}
                >
